  ...
  return (
    <div className={`py-24 ${bgColor} relative overflow-hidden`}>
      ...
      <div className="absolute inset-0">
        <div
          className={`absolute top-20 right-20 w-96 h-96 ${gradient} rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob`}
        ></div>
        <div
          className={`absolute bottom-20 left-20 w-96 h-96 ${gradient} rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob animation-delay-2000`}
        ></div>
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="space-y-6">
              {description.map((item, index) => (
                <div key={index} className="flex items-start gap-4 group">
                  <div
                    className={`w-8 h-8 rounded-full ${textColor} bg-white/80 backdrop-blur-sm flex items-center justify-center flex-shrink-0 shadow-sm group-hover:scale-110 transition-transform duration-200`}
                  >
          ...
          <div className="order-1 lg:order-2">
            <div className="relative group">
              ...
              <div
                className={`absolute inset-0 ${gradient} rounded-3xl transform rotate-3 group-hover:rotate-6 transition-transform duration-500 opacity-20`}
              ></div>
              <div
                className={`absolute inset-0 ${gradient} rounded-3xl transform -rotate-3 group-hover:-rotate-6 transition-transform duration-500 opacity-10`}
              ></div>
              ...
              <div className="relative h-[500px] rounded-3xl overflow-hidden shadow-2xl border border-white/20">
                ...
                <div className="absolute bottom-6 left-6 bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-lg border border-gray-100/50 transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`w-3 h-3 ${textColor} rounded-full animate-pulse`}
                    ></div>
                    <div>
