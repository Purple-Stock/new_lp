...
export function PreFooterCTA() {
  ...
  const title =
    language === "pt"
      ? "Simplifique sua Gestão de Inventário"
      : language === "en"
        ? "Simplify your Inventory Management"
        : "Simplifiez votre Gestion d'Inventaire";
  const subtitle =
    language === "pt"
      ? "Transforme seu negócio com o sistema mais inteligente de controle de estoque"
      : language === "en"
        ? "Transform your business with the smartest inventory control system"
        : "Transformez votre entreprise avec le système de contrôle des stocks le plus intelligent";
  const buttonText =
    language === "pt"
        ...
        <div className="rounded-3xl border border-white/70 bg-white/70 p-8 shadow-[0_25px_100px_-30px_rgba(59,7,100,0.3),0_10px_40px_-20px_rgba(0,0,0,0.08)] backdrop-blur-2xl sm:p-10 lg:p-12">
          <div className="mb-12">
            ...
            <div className="mb-12 flex flex-wrap justify-center gap-3 sm:gap-4">
              {benefits.map((benefit, index) => (
                <div
                  key={index}
                  className="flex items-center space-x-2 rounded-full border border-purple-200/80 bg-white/90 px-4 py-2 shadow-sm"
                >
          ...
          <div className="mt-14 border-t border-purple-200/70 pt-8">
            <p className="mb-6 text-sm text-slate-500">
              {language === "pt"
                ? "Já confiado por empresas líderes:"
                : language === "en"
                  ? "Already trusted by leading companies:"
                  : "Déjà approuvé par des entreprises leaders:"}
            </p>
