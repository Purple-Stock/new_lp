...
function getDefaultPayload(
  ...
  return {
    ...
    page_type:
      payload.page_type ??
      (window.location.pathname === "/" ? "home" : "content"),
    utm_source:
      payload.utm_source ?? url.searchParams.get("utm_source") ?? "direct",
    utm_medium:
      payload.utm_medium ?? url.searchParams.get("utm_medium") ?? "none",
    utm_campaign:
      payload.utm_campaign ?? url.searchParams.get("utm_campaign") ?? "none",
