...
// Configuration
const LLM_TXT_URL = process.env.LLM_TXT_URL || 'http://localhost:3001/llm.txt';
...
async function validateLlmTxt() {
  log('🤖 Starting LLM.txt validation...', 'blue');
  log(`📍 LLM.txt URL: ${LLM_TXT_URL}`, 'cyan');

  try {
    // Fetch llms.txt
    const response = await makeRequest(LLM_TXT_URL);
    ...
    if (contentType && !contentType.includes('text/plain')) {
      log(`⚠️  Content-Type should be 'text/plain', got: ${contentType}`, 'yellow');
    } else {
    ...
    if (passedChecks === totalChecks) {
      log(`   ✅ LLM.txt is fully valid! (${passedChecks}/${totalChecks})`, 'green');
    } else {
      log(`   ⚠️  LLM.txt has some issues (${passedChecks}/${totalChecks})`, 'yellow');
    }
    ...
    log(`❌ Error validating llm.txt: ${error.message}`, 'red');
  }
}
